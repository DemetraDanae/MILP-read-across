...
training_samples = randperm(84,samples); % random sampling
Test_samples=setdiff([1:84],training_samples);
...
for re=2:regions
    for i=1:samples
        constraints=[constraints,Xrf(re-1,p_f)+epsilon-U*(1-Frs(re,i))<= Asf1(i,p_f)];
    end
end 

for re=1:regions-1 
    for i=1:samples
        constraints=[constraints,Asf1(i,p_f)<=Xrf(re,p_f)-epsilon+U*(1-Frs(re,i))],
    end 
end 
...
for i=1:test_samples
    for re1=1:finalregions1
        for re2=1:finalregions2
          if Fr1s_test(re1,i)==1 && Fr2s_test(re2,i)==1
               F_test(re1,re2,i)=1;
          end 
       end
    end
...
[R2, R2_test, q2, q2_test]

for fe=1:features
    for re1=1:finalregions1
        for re2=1:finalregions2
            if abs(bestWrf(re1,re2,fe)) <= 1e-5
                bestWrf(re1,re2,fe) = 0;
            end
        end
    end
end
