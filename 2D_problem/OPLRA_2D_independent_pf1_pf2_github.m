...
Xrf=sdpvar(regions,features1);
Es=sdpvar(samples,1);
Ers=sdpvar(regions,samples);
Wrf=sdpvar(regions,features1);
Wrfpos=sdpvar(regions,features1);
MAE=sdpvar(1);
REG=sdpvar(1);
...
for re=2:regions
    constraints=[constraints,Xrf(re,p_f1)>=Xrf(re-1,p_f1)+epsilon];
end 

constraints=[constraints,Xrf(regions,p_f1)==1];
constraints=[constraints,Xrf(1,p_f1)>=epsilon];
...
for re=2:regions
    for i=1:samples
        constraints=[constraints,Xrf(re-1,p_f1)+epsilon-U*(1-Frs(re,i))<= Asf1(i,p_f1)];
    end
end 

for re=1:regions-1 
    for i=1:samples
        constraints=[constraints,Asf1(i,p_f1)<=Xrf(re,p_f1)-epsilon+U*(1-Frs(re,i))];
    end 
end 

for re=1:regions
   for i=1:samples
        constraints=[constraints,Pred(re,i)==sum(Asf1(i,:).*Wrf(re,:))+Br(re,1)];
   end 
end 
...
if value(z)<ERRORtmp1 && value(z)~=0
    ERRORtmp1=value(z);
    zopt1=value(z);
    bestMAE=value(MAE);
    bestREG=value(REG);
...
Br=sdpvar(regions,1);
Pred=sdpvar(regions,samples);
Xrf=sdpvar(regions,features2);
Es=sdpvar(samples,1);
Ers=sdpvar(regions,samples);
Wrf=sdpvar(regions,features2);
Wrfpos=sdpvar(regions,features2);
MAE=sdpvar(1);
REG=sdpvar(1);
...
for re=2:regions
    constraints=[constraints,Xrf(re,p_f2)>=Xrf(re-1,p_f2)+epsilon];
end 

constraints=[constraints,Xrf(regions,p_f2)==1];
constraints=[constraints,Xrf(1,p_f2)>=epsilon];
...
for re=2:regions
    for i=1:samples
        constraints=[constraints,Xrf(re-1,p_f2)+epsilon-U*(1-Frs(re,i))<= Asf2(i,p_f2)];
    end
end 

for re=1:regions-1 
    for i=1:samples
        constraints=[constraints,Asf2(i,p_f2)<=Xrf(re,p_f2)-epsilon+U*(1-Frs(re,i))];
    end 
end 

for re=1:regions
   for i=1:samples
        constraints=[constraints,Pred(re,i)==sum(Asf2(i,:).*Wrf(re,:))+Br(re,1)];
   end 
end 
...
if value(z)<ERRORtmp2 && value(z)~=0
    ERRORtmp2=value(z);
    zopt2=value(z);
    bestMAE=value(MAE);
    bestREG=value(REG);
...
ERRORcurrent_test=ERRORtmp_test;
ERRORcurtest_table(step,1)=ERRORcurrent_test;
step=step+1;
